...
O baseline (`.sentiness/baseline.json`) estava vazio; foi reinicializado (`sentiness baseline init`) com decisão do Yan. Suprime 25 achados pré-existentes do knip que são falsos-positivos no monorepo: deps do Nuxt em `apps/web` (`vuetify`, `@mdi/font`, `sass`, `vue-tsc`), `multer` (usado via `FilesInterceptor`, sem import direto), os pacotes `@sentiness/check-*` (invocados pela CLI) e `eslint` (cross-workspace), além de exports de schema/fixtures de scaffolding. `newInDiff` limpo — a F2 não introduziu nenhuma violação nova. Reportar ao repo Sentiness: knip precisa de config para Nuxt e para deps invocadas dinamicamente.
